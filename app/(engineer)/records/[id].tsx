...
export default function EngineerHistoryDetailScreen() {
  ...
  const [surveyAnswers, setSurveyAnswers] = useState<
    { field_key: string; field_value: string }[]
  >([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      if (source === "job") {
        const data = await apiFetch(`/api/engineer/jobs/${id}`);
        setJob(data);
        // fetch survey answers separately
        try {
          const answers = await apiFetch(`/api/engineer/jobs/${id}/answers`);
  ...
  // ── SOURCE = JOB ──
  if (source === "job" && job) {
    const statusColor = TRACKING_STATUS_COLOR[job.status] ?? "#ccc";
    const photos =
      job.attachments?.filter((a) => a.type === "tracking_doc") ?? [];
